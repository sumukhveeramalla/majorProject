...
#     noise = np.random.normal(mean,var**0.5,features.shape)
#     return np.clip(features + noise, 0, 255).astype('int')

def gaussian_noise(features):
    mean = 0
    # variance: randomly between 1 to 25
    var = torch.randint(1, 26, (1,), dtype=torch.float32)
    # sigma is square root of the variance value
    noise = torch.normal(mean, var.sqrt(), features.shape)
    return torch.clamp(features + noise, 0, 255).to(torch.uint8)

def gaus_noise(features, mean=0, std=0.01, p=0.5):
    features_gaus_noise = features.reshape(features.shape[0], features.shape[1], 50, 3)
    if random.random() < p:
        temp = features_gaus_noise.copy()
        C,T,V,M = features_gaus_noise.shape
        noise = np.random.normal(mean,std,size=(C,T,V,M))
        temp = temp + noise
        return torch.as_tensor(temp)
    else:
        return torch.as_tensor(features_gaus_noise)
